...
Class DashHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            if(e.Name == "Dash")
            {
                Let DashPlayer = BuildPlayer(Players[e.Player].Mo);
...
Class WallSlideHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        ...
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            if(e.Name == "WallSlide")
            {
                Let WSlidePlayer = BuildPlayer(Players[e.Player].Mo);
...
Class GrapplingHookHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        ...
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            ...
            {
                Let GrapplingPlayer = BuildPlayer(Players[e.Player].Mo);
...
Class Hook : Actor
{
	...
	Override void Tick()
	{
		Let HookOwner = BuildPlayer(Target);
		if(HookOwner)
		{
	...
	void InitiateGrapple(Bool Monster)
	{
		...
		if(Monster)
		{
			...
			BuildPlayer(Target).GrappledMonster = Monster;
