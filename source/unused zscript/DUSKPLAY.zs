...
Class DuskPlayer : PlayerPawn
{
	...
	Default
    {
		Player.DisplayName "Dusk Player";
        PainChance 255;
    }
...
Class DashHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            if(e.Name == "Dash")
            {
                Let DashPlayer = DuskPlayer(Players[e.Player].Mo);
...
Class WallSlideHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        ...
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            if(e.Name == "WallSlide")
            {
                Let WSlidePlayer = DuskPlayer(Players[e.Player].Mo);
...
Class GrapplingHookHandler : EventHandler
{
    Override void NetworkProcess(ConsoleEvent e)
    {
        ...
        if(e.Player >= 0 && PlayerInGame[e.Player] && Players[e.Player].Mo)
        {
            ...
            {
                Let GrapplingPlayer = DuskPlayer(Players[e.Player].Mo);
...
Class Hook : Actor
{
	...
	Override void Tick()
	{
		Let HookOwner = DuskPlayer(Target);
		if(HookOwner)
		{
	...
	void InitiateGrapple(Bool Monster)
	{
		...
		if(Monster)
		{
			...
			DuskPlayer(Target).GrappledMonster = Monster;
